...
def run_ship(image_bytes: bytes, model_path: str = None) -> bytes:
    ...
    # 3️⃣ Convert to OpenCV for crisp box drawing
    out_cv = cv2.cvtColor(np.array(pil_img), cv2.COLOR_RGB2BGR)

        # 4️⃣ Draw blue boxes (BGR = (255, 0, 0))
    for (x1, y1, x2, y2, conf) in boxes:
        cv2.rectangle(
            out_cv,
            (int(x1), int(y1)),
            (int(x2), int(y2)),
            (255, 0, 0), 3  # Blue box, thickness=2
        )
        # (No text labels for cleaner presentation)
    ...
    # 6️⃣ Convert back to PIL for return
    out_pil = Image.fromarray(cv2.cvtColor(out_cv, cv2.COLOR_BGR2RGB))
    buf = io.BytesIO()
